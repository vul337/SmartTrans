...
def construct_cfg(filepath) -> Tuple[Mapping[str, Block], Mapping[str, Function]]:
    # Load facts
    tac_function_blocks = load_csv_multimap(f'{filepath}/InFunction.csv', reverse=True)

    tac_func_id_to_public = load_csv_map(f'{filepath}/PublicFunction.csv')
    tac_high_level_func_name = load_csv_map(f'{filepath}/HighLevelFunctionName.csv')

    tac_formal_args: Mapping[str, List[Tuple[str, int]]] = defaultdict(list)
    for func_id, arg, pos in load_csv(f'{filepath}/FormalArgs.csv'):
    ...
    tac_block_stmts = load_csv_multimap(f'{filepath}/TAC_Block.csv', reverse=True)
    tac_op = load_csv_map(f'{filepath}/TAC_Op.csv')

    # Load statement defs/uses
    tac_defs: Mapping[str, List[Tuple[str, int]]] = defaultdict(list)
    for stmt_id, var, pos in load_csv(f'{filepath}/TAC_Def.csv'):
        tac_defs[stmt_id].append((var, int(pos)))

    tac_uses: Mapping[str, List[Tuple[str, int]]] = defaultdict(list)
    for stmt_id, var, pos in load_csv(f'{filepath}/TAC_Use.csv'):
        tac_uses[stmt_id].append((var, int(pos)))

    # Load block edges
    tac_block_succ = load_csv_multimap(f'{filepath}/LocalBlockEdge.csv')
    ...
    for block_id, in load_csv(f'{filepath}/IRFunctionEntry.csv'):
